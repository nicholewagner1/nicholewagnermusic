...
/* CONFIGURATION FUNCTIONS */
	...
	public function shortcode_display($atts, $content=null, $tag){
	    ...
	    $gallery = (!isset($a["gallery"]) || strtolower($a["gallery"]) == "false")? false : true;
		...
		if ( ! is_null($values) && ! empty($values) ){
			...
			if ( is_airpress_record($values[0]) ){
				...
				if ( isset($a["gallery"])){
					...
					foreach($values as $value){
						//$output_values[] = $value;
						$output_values[] = "<a href=".$value."><img src=".$value." width=200 target=new></a>";
