...
class AudioTheme_ThemeNotice {
	...
	public function __construct( $args = array() ) {
		if ( ! $this->is_audiotheme_active() && is_admin() && current_user_can( 'activate_plugins' ) ) {
			$this->strings = array(
				'notice'     => esc_html__( 'The AudioTheme plugin should be installed and activated for this theme to display properly.', 'wayfarer' ),
				'activate'   => esc_html__( 'Activate now', 'wayfarer' ),
				'learn_more' => esc_html__( 'Find out more', 'wayfarer' ),
				'dismiss'    => esc_html__( 'Dismiss', 'wayfarer' ),
			);
	...
	public function display_notice() {
		...
		jQuery( '#audiotheme-required-notice' ).on( 'click', '.dismiss', function( e ) {
			...
			jQuery.get( ajaxurl, {
				action: '<?php echo $this->dismiss_notice_action(); // phpcs:ignore ?>',
				_wpnonce: '<?php echo wp_create_nonce( $this->dismiss_notice_action() ); // phpcs:ignore ?>'
			}, function() {
				$notice.slideUp();
			});
		});
