...
class AudioTheme_ThemeNotice {
	...
	public function __construct( $args = array() ) {
		if ( ! $this->is_audiotheme_active() && is_admin() && current_user_can( 'activate_plugins' ) ) {
			$this->strings = array(
				'notice'     => esc_html__( 'The AudioTheme plugin should be installed and activated for this theme to display properly.', 'encore' ),
				'activate'   => esc_html__( 'Activate now', 'encore' ),
				'learn_more' => esc_html__( 'Find out more', 'encore' ),
				'dismiss'    => esc_html__( 'Dismiss', 'encore' ),
			);
