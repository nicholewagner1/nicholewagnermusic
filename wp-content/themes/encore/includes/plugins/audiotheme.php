...
function encore_audiotheme_enqueue_assets() {
	...
	wp_style_add_data( 'encore-audiotheme', 'rtl', 'replace' );

	if ( in_array( get_post_type(), array( 'audiotheme_record', 'audiotheme_track' ) ) ) {
		wp_enqueue_script( 'jquery-cue' );
	}
}
add_action( 'wp_enqueue_scripts', 'encore_audiotheme_enqueue_assets' );
 ...
 */
function encore_audiotheme_track_title( $post = 0, $args = array() ) {
	$post = get_post( $post );

	if ( function_exists( 'get_audiotheme_track_title' ) ) {
		$title = get_audiotheme_track_title( $post, $args );
	} else {
		$title = sprintf(
	...
	echo encore_allowed_tags( $title );
}
