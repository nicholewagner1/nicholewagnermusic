...
function encore_comment_navigation() {
	...
	<nav class="navigation comment-navigation" role="navigation">
		...
		<div class="nav-links">
			<?php
			$prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'encore' ) );
			$next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'encore' ) );
...
function encore_content_navigation() {
	if ( is_singular() ) :
		the_post_navigation( array(
			'prev_text' => _x( 'Prev <span class="screen-reader-text">Post: %title</span>', 'Previous post link', 'encore' ),
			'next_text' => _x( 'Next <span class="screen-reader-text">Post: %title</span>', 'Next post link', 'encore' ),
		) );
	else :
		the_posts_navigation( array(
			'prev_text' => esc_html__( 'Older', 'encore' ),
			'next_text' => esc_html__( 'Newer', 'encore' ),
		) );
	endif;
}
endif;
...
function encore_page_links() {
	...
	wp_link_pages( array(
		'before'      => '<nav class="page-links padded-content"><span class="page-links-title">' . esc_html__( 'Pages', 'encore' ) . '</span>',
		'after'       => '</nav>',
		'link_before' => '<span class="page-links-number">',
		'link_after'  => '</span>',
		'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'encore' ) . ' </span>%',
...
function encore_posted_by() {
	...
	<span class="posted-by byline">
		...
		echo encore_allowed_tags( sprintf(
			...
			__( '<span class="sep">by</span> %s', 'encore' ),
			encore_get_entry_author()
		) ); // WPCS: XSS OK.
		?>
	</span>
	<?php
...
function encore_posted_on() {
	...
	<span class="posted-on">
		<span class="sep"><?php echo esc_html_x( 'on', 'post date', 'encore' ); ?></span>
...
function encore_entry_comments_link() {
	...
	comments_popup_link(
		esc_html__( 'Leave a comment', 'encore' ),
		esc_html__( '1 Comment', 'encore' ),
		esc_html__( '% Comments', 'encore' )
	);
	echo '</span>';
}
endif;
...
function encore_get_entry_terms( $taxonomies = array(), $post = null ) {
	$default = array(
		'category' => esc_html__( 'Posted In:', 'encore' ),
		'post_tag' => esc_html__( 'Tagged:', 'encore' ),
	);
...
function encore_get_credits() {
	$text = sprintf(
		__( 'Powered by <a href="%1$s">AudioTheme</a>', 'encore' ),
